...
lit_raw <- 
  readxl::read_xlsx("G:/My Drive/Research/Projects/SR_meta_analysis/data/lit_review_2025.xlsx", sheet = "data_raw")
...
treatment_cols <- setdiff(grep("^T_", names(lit), value = T), "T_rep_n")
...
#write.csv(lit_effect, "G:/My Drive/Research/Projects/SR_meta_analysis/Data/Ln_RR_raw.csv", row.names = F)
```
...
#write.csv(summary_table, "G:/My Drive/Research/Projects/SR_meta_analysis/Data/Ln_RR_summary.csv", row.names = F)

#summary_table <- read.csv("G:/My Drive/Research/Projects/SR_meta_analysis/Data/Ln_RR_summary.csv")
```
...
summary_table_selected <- 
  summary_table %>%
  filter(!Response %in% c("ln_FRP", "ln_CRP", "ln_BGB", "ln_MBN", "ln_Clay", "ln_Silt", "ln_Sand", "ln_DOC", "ln_DON"))

# Define factor levels to control y-axis order
response_levels <- 
  summary_table_selected %>%
  distinct(Response) %>%
  pull(Response) %>%          # extracts as a character vector
  str_remove("^ln_") %>%      # removes "ln_" prefix
  rev() 
...
#write.csv(NA_ratios, "G:/My Drive/Research/Projects/SR_meta_analysis/Data/NA_ratios.csv", row.names = F)
```
...
```{r corr every vars}
env_var <- 
  setdiff(Response_vars, vars) %>% 
  setdiff(., c("ln_Silt", "ln_Sand", "ln_Clay", "ln_FRP", "ln_CRP"))

management_types <- c("CC", "PB", "TH")
...
plot_chunks_CC <- split(plot_list_CC, ceiling(seq_along(plot_list_CC) / 20))
...
# Break the plot list into 3 groups of 25
plot_chunks_PB <- split(plot_list_PB, ceiling(seq_along(plot_list_PB) / 20))
...
# Break the plot list into 3 groups of 25
plot_chunks_TH <- split(plot_list_TH, ceiling(seq_along(plot_list_TH) / 20))
...
p_CC_SR_Ts <- 
  ggplot(CC, aes(ln_Ts, ln_SR)) + 
  geom_hline(yintercept = 0, color = "black", size = 1, alpha = 0.4) +
  ...
  stat_poly_eq(
    ...
    size = 5,
    parse = T,
    label.x.npc = "left",    # "left", "center", "right" or 0–1
    label.y.npc = "bottom",  # "top", "bottom", "center" or 0–1
    ) +
  theme_classic() +
  guides(size = "none") +
  xlab("lnRR(Ts)") + ylab("lnRR(SR)") +
  theme_corr_template +
  scale_y_continuous(labels = function(x) sub("-", "\u2212", x)) 

p_CC_SR_SM <- 
  ggplot(CC, aes(ln_SM, ln_SR)) + 
  geom_hline(yintercept = 0, color = "black", size = 1, alpha = 0.4) +
  geom_vline(xintercept = 0, color = "black", size = 1, alpha = 0.4) +
  geom_point(aes(size = Wt), shape = 1) + 
  geom_smooth(method = lm, se = TRUE) + 
  ...
  theme_classic() +
  guides(size = "none") +
  xlab("lnRR(SM)") + ylab("lnRR(SR)") +
  theme_corr_template +
  scale_x_continuous(labels = function(x) sub("-", "\u2212", x)) +
  scale_y_continuous(labels = function(x) sub("-", "\u2212", x)) 

p_CC_SR_MBC <- 
  ggplot(CC, aes(ln_MBC, ln_SR)) + 
  geom_hline(yintercept = 0, color = "black", size = 1, alpha = 0.4) +
  geom_vline(xintercept = 0, color = "black", size = 1, alpha = 0.4) +
  geom_point(aes(size = Wt), shape = 1) + 
  geom_smooth(method = lm, se = TRUE) + 
...
p_TH_SR_FF <- 
  ...
  stat_poly_eq(
    ...
    parse = T,
    label.x.npc = "left",    # "left", "center", "right" or 0–1
    label.y.npc = "top",  # "top", "bottom", "center" or 0–1
    ) +
  theme_classic() +
  guides(size = "none") +
  xlab("lnRR(FF)") + ylab("lnRR(SR)") +
  theme_corr_template +
  scale_x_continuous(labels = function(x) sub("-", "\u2212", x)) +
  scale_y_continuous(labels = function(x) sub("-", "\u2212", x))

p_TH_Rh_MBC <- 
  ggplot(TH, aes(ln_MBC, ln_Rh)) + 
  geom_hline(yintercept = 0, color = "black", size = 1, alpha = 0.4) +
  geom_vline(xintercept = 0, color = "black", size = 1, alpha = 0.4) +
  geom_point(aes(size = Wt), shape = 1) + 
  geom_smooth(method = lm, se = TRUE) + 
  ...
  theme_classic() +
  guides(size = "none") +
  xlab("lnRR(MBC)") + ylab("lnRR(Rh)") +
  theme_corr_template +
  scale_x_continuous(labels = function(x) sub("-", "\u2212", x)) +
  scale_y_continuous(labels = function(x) sub("-", "\u2212", x))

p_TH_Rh_LF <- 
  ggplot(TH, aes(ln_LF, ln_Rh)) + 
  geom_hline(yintercept = 0, color = "black", size = 1, alpha = 0.4) +
  geom_vline(xintercept = 0, color = "black", size = 1, alpha = 0.4) +
  geom_point(aes(size = Wt), shape = 1) + 
  geom_smooth(method = lm, se = TRUE) + 
  ...
  theme_classic() +
  guides(size = "none") +
  xlab("lnRR(LF)") + ylab("lnRR(Rh)") +
  theme_corr_template +
  scale_x_continuous(labels = function(x) sub("-", "\u2212", x)) +
  scale_y_continuous(labels = function(x) sub("-", "\u2212", x))

p_TH_Ra_FRB <- 
  ggplot(TH, aes(ln_FRB, ln_Ra)) + 
  geom_hline(yintercept = 0, color = "black", size = 1, alpha = 0.4) +
  geom_vline(xintercept = 0, color = "black", size = 1, alpha = 0.4) +
  geom_point(aes(size = Wt), shape = 1) + 
  geom_smooth(method = lm, se = TRUE) + 
  ...
  theme_classic() +
  guides(size = "none") +
  xlab("lnRR(FRB)") + ylab("lnRR(Ra)") +
  theme_corr_template +
  scale_x_continuous(labels = function(x) sub("-", "\u2212", x)) +
  scale_y_continuous(labels = function(x) sub("-", "\u2212", x))

# Define layout using `area()` — specify positions manually
layout <- c(
  area(1, 1, 1, 1),  # row 1, col 1
  area(2, 1, 2, 1),  # row 2, col 1
  area(3, 1, 3, 1),  # row 3, col 1
  area(1, 2, 1, 2),  # row 1, col 2
  area(2, 2, 2, 2),  # row 2, col 2
  area(3, 2, 3, 2),  # row 3, col 2
  area(4, 2, 4, 2)   # row 4, col 2 (extra row only in col 2)
)
...
corr_fig_dir <- "G:/My Drive/Research/Projects/SR_meta_analysis/Fig/corr/"
