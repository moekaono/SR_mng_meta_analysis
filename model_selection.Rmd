...
res_CC_SR <- 
  glmulti(
    "ln_SR", xr = c(prednames, "Residue"), 
    data = CC %>% select(c(prednames, "Residue"), ln_SR) %>% mutate(across(where(~ !is.numeric(.)), as.factor)), 
    level = 1,
    crit = "aicc",
    plotty = F
    )
...
res_PB_SR <- 
  glmulti(
    ...
    data = PB %>% select(all_of(c(prednames, "Severity_level", "Repeat")), ln_SR, Severity_level, Repeat) %>% mutate(across(where(~ !is.numeric(.)), as.factor)), 
    level = 1,
    crit = "aicc",
    plotty = F
    )
...
res_TH_SR <- 
  glmulti(
    "ln_SR", xr = c(prednames, "Severity_level"), 
    data = TH %>% select(all_of(c(prednames, "Severity_level")), ln_SR, Severity_level, Repeat) %>% mutate(across(where(~ !is.numeric(.)), as.factor)), 
    level = 1,
    crit = "aicc",
    plotty = F
    )
